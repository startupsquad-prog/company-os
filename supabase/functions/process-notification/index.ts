  ...
  try {
    ...
    if (!supabaseServiceKey) {
      ...
      return new Response(
        JSON.stringify({
          success: false,
          error: "Missing SUPABASE_SERVICE_ROLE_KEY. Please set it in Supabase Dashboard > Edge Functions > Secrets",
        }),
        { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 500 }
      );
    }
    ...
    if (!payload.entity_type || !payload.entity_id || !payload.action || !payload.notification_type) {
    ...
    try {
      ...
      console.warn("⚠️ [Edge Function] Failed to fetch entity/actor data, using defaults", { error });
    }

    // Step 3: Build notification message
    const { title, message } = payload.custom_title && payload.custom_message
    ...
    if (filteredRecipients.length === 0) {
      console.log("⚠️ [Edge Function] All recipients already received this notification recently, skipping");
      return new Response(
        JSON.stringify({
          success: true,
          notifications_created: 0,
